...
$(function(){
    ...
    $('input[name=name]').blur(function(){
        val = this.value;
        if (val.length<2 ||val.length>10 || !val.match(/^([a-zA-Z0-9]|[\u4e00-\u9fa5]|[._]){1,10}$/)){
            $(this).next().show();
        }else {
            $(this).next().hide();
        }
...
//改密码验证
    $('input[name=gpassword]').blur(function(){
        val = this.value;
        if (val.length<4 || $('#password').val().length > 10 || !val.match(/^[^ ]+$/)){
            $(this).next().show();
        }else {
            $(this).next().hide();
        }
...
//点击注销事件返回登录页面
    ...
    $(document).ready(function(){
        $.ajax({
            ...
            success: function(r) {
                if (r.success==true){
                    //cjy_alert(r.userProfile.usertype);
                    $.cookie('usertypes', r.userProfile.usertype);
                    if(r.userProfile.usertype != 1){
                        $("#chaxun,#tianjia,#xiugai,#shanchu").click(function(){
                            cjy_alert("非管理员用户只能更新密码！")
                            return false;
                        });
                        $("#qpageBox").remove();
                        $("#chaxun").ready(function(){
                            $.ajax({
                                type:'POST',
...
//                        修改时间格式的转换
    ...
    $("#xiugai").click(function(){
        if ($("input[name='ckb']:checked").length != 1){
            ...
            if ($(':checkbox:checked').parent().next().next().next().text()=="用户"){
                $("#gusertype").val(3);
            }else if($(':checkbox:checked').parent().next().next().next().text()=="操作员"){
                $("#gusertype").val(2);
            }else {
                $("#gusertype").val(1);
            }
            $("#gname").val($(':checkbox:checked').parent().next().next().text());
            //cjy_alert($(':checkbox:checked').parent().next().next().next().text())
            $("#gEmail1").val($(':checkbox:checked').parent().next().next().next().next().next().text());
            $("#gphone").val($(':checkbox:checked').parent().next().next().next().next().text());
            //cjy_alert($(':checkbox:checked').parent().next().next().text());
        }
    });
    $("#xgsj_post").click(function(){
        if($('#gname').val().length <= 1 || $('#gname').val().length >= 11 || !$('#gname').val().match(/^([a-zA-Z0-9]|[\u4e00-\u9fa5]){1,10}$/)){
            $ ('#gname').next().show();
                ...
                success: function(r) {
                    if(r.success==false){
                        if (r.message=="_id not exist"){
                            cjy_alert("账号不存在！")
                        }else if (r.message=="Non-admin user can only update password"){
                            cjy_alert("非管理员用户只能更新密码!");
                        } else {
    ...
    $("#gmsj_post").click(function(attr){
        if ($('#gpassword').val() == "" || $('#gpassword').val().length <= 3 || $('#gpassword').val().length>=11 || !val.match(/^[^ ]+$/)) {
            $ ('#gpassword').next().show();
